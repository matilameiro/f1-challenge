...
import "./chunk-TFYDG3RZ.js";
import "./chunk-VZUAAQTR.js";
import {
  NzOutletModule,
...
var MenuService = class _MenuService {
  ...
  descendantMenuItemClick$ = new import_rxjs.Subject();
  /** child menu item click **/
  childMenuItemClick$ = new import_rxjs.Subject();
  theme$ = new import_rxjs.BehaviorSubject("light");
  mode$ = new import_rxjs.BehaviorSubject("vertical");
  inlineIndent$ = new import_rxjs.BehaviorSubject(24);
  isChildSubMenuOpen$ = new import_rxjs.BehaviorSubject(false);
...
var NzSubmenuService = class _NzSubmenuService {
  ...
  mode$ = this.nzMenuService.mode$.pipe((0, import_operators.map)((mode) => {
    if (mode === "inline") {
      return "inline";
  ...
  isCurrentSubMenuOpen$ = new import_rxjs.BehaviorSubject(false);
  isChildSubMenuOpen$ = new import_rxjs.BehaviorSubject(false);
  /** submenu title & overlay mouse enter status **/
  isMouseEnterTitleOrOverlay$ = new import_rxjs.Subject();
  childMenuItemClick$ = new import_rxjs.Subject();
  ...
  constructor() {
    ...
    const isClosedByMenuItemClick = this.childMenuItemClick$.pipe((0, import_operators.mergeMap)(() => this.mode$), (0, import_operators.filter)((mode) => mode !== "inline" || this.isMenuInsideDropDown), (0, import_operators.map)(() => false));
...
var NzMenuItemComponent = class _NzMenuItemComponent {
  ...
  selected$ = new import_rxjs.Subject();
...
var NzMenuDirective = class _NzMenuDirective {
  ...
  actualMode = "vertical";
  dir = "ltr";
  inlineCollapsed$ = new import_rxjs.BehaviorSubject(this.nzInlineCollapsed);
  mode$ = new import_rxjs.BehaviorSubject(this.nzMode);
  destroy$ = new import_rxjs.Subject();
